...
def bandwidth_type(x):
	...
	if( xx < 34 ):
		...
		raise argparse.ArgumentTypeError("Maximum min-length should be 150")
	return xx


logger = logging.getLogger("simpTrimPrinseqLite_R1andR2.py")
logger.setLevel(logging.INFO)

parser = argparse.ArgumentParser(description="Wrapper for prinseq-lite", usage="python windowQualPrinseqLite_R1andR2.py inputPath/reads_R1_001.fastq inputPath/reads_R2_001.fastq --rm_ambig Y/N --min_len 100 --trim_qual [Y/N] --min_score 28 --qual_window 20 --clean_output [Y/N] --outDir outputPath")

## minimum read length
parser.add_argument('--min_len', '-min', type=bandwidth_type, default=100, help="Remove reads less than --min_len: Default=100, 39 < --min_len < 151")
...
parser.add_argument('--fixTrim', '-fixed', default='N', choices=['Y','N'], help="trim by base pair quality (Y|N)?")
parser.add_argument('--trim', default=3, type=int, choices=range(1, 20), help="R1 trim at 3-prime [1 to 20: default 3]")
#parser.add_argument('--revtrim', default=5, type=int, choices=range(1, 20), help="sliding window size used to calculate quality score [10 to 45: default 20]")
...
def getIsolateStr(filePathString, format):
	...
	if(format == 1):
		...
		if(re.search(pattern='_R1_001\.fastq', string=isolateString[0])):
			fileString = re.sub(r'R1_001\.fastq', 'winPrinseq', isolateString[0])
		else:
			fileString = isolateString[0] + '_winPrinseq'
	#print(fileString)
	return fileString
...
logger.info("starting prinseq-lite")

if(re.search(r'\.fastq$', forward, flags=re.IGNORECASE) and re.search(r'\.fastq$', reverse, flags=re.IGNORECASE)):
	if((args.rm_ambig == 'Y') and (args.fixTrim == 'Y')):	
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} -trim_ns_left 1 -trim_ns_right 1 -ns_max_n {} -trim_right {} 2> {}".format(forward, reverse, outputSuccess, outputFAIL, intMinLen, intTrimN, intTrim, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads with > {} Ns removed/trimmed; reads 3-prime end {} trimmed; reads less than {} bp length removed' > {}".format(intTrimN, intTrim, intMinLen, outputLog))
	elif((args.rm_ambig == 'N') and (args.fixTrim == 'Y')):	
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} -trim_right {} 2> {}".format(forward, reverse, outputSuccess, outputFAIL, intMinLen, intTrim, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads 3-prime end {} trimmed; reads less than {} bp length removed' > {}".format(intTrim, intMinLen, outputLog))
	elif((args.rm_ambig == 'Y') and (args.fixTrim == 'N')):	
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} -trim_ns_left 1 -trim_ns_right 1 -ns_max_n {} 2> {}".format(forward, reverse, outputSuccess, outputFAIL,intMinLen, intTrimN, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads with > {} Ns removed/trimmed; reads less than {} bp length removed' > {}".format(intTrimN, intMinLen, outputLog))
	else:
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} 2> {}".format(forward, reverse, outputSuccess, outputFAIL, intMinLen, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads less than {} bp length removed' > {}".format(intMinLen, outputLog))
	...
	### Begin performing read clean
	if((args.rm_ambig == 'Y') and (args.fixTrim == 'Y')):
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} -trim_ns_left 1 -trim_ns_right 1 -ns_max_n {} -trim_right {} 2> {}"
.format(gunzipForward, gunzipReverse, outputSuccess, outputFAIL,intMinLen, intTrimN, intTrim, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads with > {} Ns removed/trimmed; reads 3-prime {} trimmed; reads less than {} bp length removed' > {}".format(intTrimN, intTrim, intMinLen, outputLog))
	elif((args.rm_ambig == 'N') and (args.fixTrim == 'Y')):
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} -trim_right {} 2> {}"
.format(gunzipForward, gunzipReverse, outputSuccess, outputFAIL,intMinLen, intMinScore, intWindow, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads 3-prime end {} trimmed; reads less than {} bp length removed' > {}".format(intTrim, intMinLen, outputLog))
	elif((args.rm_ambig == 'Y') and (args.fixTrim  == 'N')):	
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} -trim_ns_left 1 -trim_ns_right 1 -ns_max_n {} 2> {}"
.format(gunzipForward, gunzipReverse, outputSuccess, outputFAIL, intMinLen, intTrimN, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads with > {} Ns removed/trimmed; reads less than {} bp length removed' > {}".format(intTrimN, intMinLen, outputLog))
	else:
		os.system("prinseq-lite -fastq {} -fastq2 {} -out_good {} -out_bad {} -min_len {} 2> {}".format(gunzipForward, gunzipReverse, outputSuccess, outputFAIL, intMinLen, runLog))
		outputLog = newOutputFolder + "/parameters.log"
		os.system("echo 'prinseq-lite: reads less than {} bp length removed' > {}".format(intMinLen, outputLog))
...
logger.info("prinseq-lite trimming complete")
